...
const MenuSection = ({ title, items, colorClass }: { title: string, items: { name: string, desc: string, price: string }[], colorClass: string }) => (
    <div className="mb-12 last:mb-0">
        <h3 className={`text-3xl md:text-5xl font-serif mb-8 text-[var(--color-pop-dark)] uppercase tracking-tight`}>
            <span className={`bg-[${colorClass}] px-2 text-white transform -skew-x-6 inline-block`}>{title}</span>
        </h3>
        <div className="grid md:grid-cols-2 gap-6">
            {items.map((item, index) => (
                <div key={index} className="bg-white border-2 border-[var(--color-pop-dark)] rounded-2xl p-6 shadow-[4px_4px_0px_0px_var(--color-pop-dark)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_var(--color-pop-dark)] transition-all">
